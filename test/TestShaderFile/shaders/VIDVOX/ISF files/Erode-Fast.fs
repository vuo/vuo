...
void main()
{
	...
	if (PASSINDEX != 3)	{
		...
		for (float i=0.; i<=localBlurRadius; ++i)	{
			if (PASSINDEX==0)	{
				...
				tmpCoord = vec2(vv_FragNormCoord.x, clamp(vv_FragNormCoord.y+(i*pixelWidth.y/2.), 0., 1.));
	...
	//	last pass
		...
		vec4		origPixel = IMG_NORM_PIXEL(inputImage,vv_FragNormCoord);
		vec4		erodedPixel = IMG_NORM_PIXEL(quarterSizePassB,vv_FragNormCoord);
		vec4		finalPixel = max(origPixel,erodedPixel);
