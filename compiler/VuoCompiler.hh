...
private:
	...
	/**
	 * Information about a module file that the compiler has seen, though not necessarily loaded.
	 */
	class ModuleInfo
	{
	public:
	...
	private:
		...
		map<string, map<string, VuoFileUtilities::File *> > compositionFilesAtSearchPath;  ///< The subcomposition files seen (but not necessarily loaded) from moduleSearchPaths.
		...
		VuoCompilerModule * findModuleThreadUnsafe(const string &moduleKey);
		map<string, VuoFileUtilities::File *> listModulesAtSearchPath(const string &path);
		map<string, VuoFileUtilities::File *> listCompositionsAtSearchPath(const string &path);
		set<ModuleInfo> listFiles(const set<string> &moduleKeys, map<string, VuoFileUtilities::File *> (VuoCompiler::Environment::*listFiles)(const string &));
		set<ModuleInfo> listAllFiles(map<string, VuoFileUtilities::File *> (VuoCompiler::Environment::*listFiles)(const string &));
		set<ModuleInfo> listModulesThreadUnsafe(const set<string> &moduleKeys);
		set<ModuleInfo> listAllModulesThreadUnsafe(void);
		static set<ModuleInfo> listModulesInEnvironmentsThreadUnsafe(Environment *myEnvironment, Environment *sharedEnvironment, const set<string> &moduleKeys);
		VuoCompilerModule * loadModule(const ModuleInfo &moduleInfo);
	...
	public:
		...
		void loadModulesIntoCombinedEnvironment(Environment *myEnvironment, Environment *sharedEnvironment, list<ModuleInfo> modulesToLoad);
		...
		set<ModuleInfo> listModules(const set<string> &moduleKeys);
		set<ModuleInfo> listAllModules(void);
		set<ModuleInfo> listSubcompositions(const set<string> &nodeClassNames);
		set<ModuleInfo> listAllSubcompositions(void);
		static set<ModuleInfo> listModulesInEnvironments(Environment *myEnvironment, Environment *sharedEnvironment, const set<string> &moduleKeys);
		static set<ModuleInfo> listAllModulesInEnvironments(Environment *myEnvironment, Environment *sharedEnvironment);
		...
		void addCachedModuleFileToSharedEnvironment(const string &moduleRelativePath);
		void removeCachedModuleFileFromSharedEnvironment(const string &moduleKey);
		void removeCachedModulesForDeletedSubcompositions(void);
		void addSubcompositionFileToSharedEnvironment(const string &subcompositionPath);
		void removeSubcompositionFileFromSharedEnvironment(const string &nodeClassName);
	};
...
private:
	...
	bool shouldLoadAllModules;  ///< If true, all modules are loaded the first time any module needs to be loaded. If false, modules are loaded only as needed.
	bool hasLoadedAllModules;  ///< Whether all modules have been loaded as a result of shouldLoadAllModules being true upon loading a module.
	map< string, set<string> > subcompositionsWithinSubcomposition;  ///< Dependency graph of subcompositions, indexed by node class namde.
	dispatch_queue_t modulesToLoadQueue;  ///< Synchronizes access to shouldLoadAllModules, hasLoadedAllModules, and subcompositionsWithinSubcomposition.
	static map<string, pair<string, VuoCompilerException> > errorsLoadingModules;  ///< Errors encountered while loading modules, along with the module file paths, indexed by module key.
	static dispatch_queue_t errorsLoadingModulesQueue;  ///< Synchronizes access to errorsLoadingModules.
	...
	void updateSubcompositionsWithinSubcomposition(const map<string, ModuleInfo> &moduleInfoForSubcomposition);
	void reifyGenericPortTypes(VuoCompilerComposition *composition);
	void reifyGenericPortTypes(VuoNode *node);
	bool compileSubcompositionIfNeeded(const string &compositionPath);
	void uninstallSubcomposition(string nodeClassName);
	void uninstallSubcompositionModule(string nodeClassName);
...
public:
	...
	string getLicenseUsername(void);
	static Module * readModuleFromBitcode(string inputPath);
	static void deleteModule(Module *module);
	static set<string> getEncounteredPremiumModules(void);
	map<string, VuoCompilerException> checkForErrorsLoadingModules(void);
