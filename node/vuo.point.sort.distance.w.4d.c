...
VuoModuleMetadata({
					  "title" : "Sort 4D Points by W Distance",
					  "description" :
						  "<p>Puts a list of points in order of nearest to farthest from a target point, \
						  comparing only the difference in w-coordinates.</p> \
						  ",
...
)
{
	*sorted = VuoListCreate_VuoPoint4d();
	...
	for(int i = 0; i < count; i++)
		pointValues[i] = (sortable_pointValue){i, fabs(VuoListGetValueAtIndex_VuoPoint4d(list, i+1).w - point.w)};
