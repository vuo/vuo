...
@interface VuoWindowOpenGLInternal : NSWindow <NSWindowDelegate>
{
	...
	CVDisplayLinkRef displayLink;	///< The display link, created when -enableTriggers... is called.
	struct VuoWindowOpenGLInternal_displayLinkContext *displayLinkContext;	///< Context data for the display link callback, created when the window is initialized (so that it persists through live-coding reloads).
...
- (void)enableTriggersWithRequestedFrame:(void (*)(VuoFrameRequest))requestedFrame
							movedMouseTo:(void (*)(VuoPoint2d))movedMouseTo
						   scrolledMouse:(void (*)(VuoPoint2d))scrolledMouse
						 usedMouseButton:(void (*)(VuoMouseButtonAction))usedMouseButton;
- (void)disableTriggers;
- (void)scheduleRedraw;
