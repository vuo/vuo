...
VuoModuleMetadata({
					  ...
					  "description" :
						  "<p>Puts the hands in order of nearest to farthest from a target point.</p> \
...
)
{
	*sortedHands = VuoListCreate_VuoLeapHand();
	...
	for(int i = 0; i < count; i++)
		pointValues[i] = (sortable_pointValue){i, VuoPoint3d_squaredMagnitude(VuoPoint3d_subtract(VuoListGetValueAtIndex_VuoLeapHand(Hands, i+1).palmPosition, target))};
