...
VuoModuleMetadata({
					 ...
					 "description" :
						"<p>Repeatedly fires events at a steady rate.</p> \
						<p>When the composition starts or this node is added to a composition, it begins firing every `seconds` seconds.</p> \
						<p>When this node receives an event, the timer resets. The next time it fires will be `seconds` seconds later.</p>",
