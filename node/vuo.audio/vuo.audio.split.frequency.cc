...
VuoAudioSamples SamplesWithFilterAndFrequency(const VuoAudioSamples audioSamples, gam::Biquad<> *filter, const double frq)
{
	(*filter).freq(frq);

	VuoAudioSamples filteredSamples = VuoAudioSamples_alloc(audioSamples.sampleCount);
	unsigned int sampleCount = audioSamples.sampleCount;
	filteredSamples.samplesPerSecond = audioSamples.samplesPerSecond;

	if( (*filter).type() == gam::BAND_PASS)
	{
		for(int i = 0; i < sampleCount; i++)
			filteredSamples.samples[i] = (*filter).nextBP( audioSamples.samples[i] );
	}
	else
	{
		for(int i = 0; i < sampleCount; i++)
			filteredSamples.samples[i] = (*filter)( audioSamples.samples[i] );
	}
...
)
{
	...
	*splitSamples = VuoListCreate_VuoAudioSamples();
	...
	for(int i = 0; i < len; i++)
		freq[i] = VuoListGetValueAtIndex_VuoReal(cutoffFrequencies, i+1);
	...
	VuoListAppendValue_VuoAudioSamples( (*splitSamples), SamplesWithFilterAndFrequency( audioSamples, low, freq[0]) );
	...
	if(len > 1)
	{
		for(int n = 0; n < len-1; n++)
		{
			...
			VuoListAppendValue_VuoAudioSamples( (*splitSamples), SamplesWithFilterAndFrequency( audioSamples, mid, (highCutoff+lowCutoff)/2.) );
		}
	}

	VuoListAppendValue_VuoAudioSamples( (*splitSamples), SamplesWithFilterAndFrequency( audioSamples, high, freq[len-1]) );
