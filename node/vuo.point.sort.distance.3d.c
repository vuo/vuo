...
VuoModuleMetadata({
					  "title" : "Sort 3D Points by Distance",
					  "description" :
						  "<p>Puts a list of points in order of nearest to farthest from a target point.</p> \
						  ",
...
)
{
	*sorted = VuoListCreate_VuoPoint3d();
	...
	for(int i = 0; i < count; i++)
		pointValues[i] = (sortable_pointValue){i, VuoPoint3d_squaredMagnitude(VuoPoint3d_subtract(VuoListGetValueAtIndex_VuoPoint3d(list, i+1), point))};
