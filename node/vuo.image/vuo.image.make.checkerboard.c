...
static const char * checkerboardFragmentShaderSource = VUOSHADER_GLSL_SOURCE(120,
	...
	void main()
	{
		...
		vec2 checkPos = fract(c + filterWidth/2) * gridType;
		...
		gl_FragColor = mix(mixed, mix(color1,color2,0.5), crossfadeAmount);
	}
);
...
)
{
	...
	VuoShader_setUniform_VuoReal   ((*instance)->shader, "sharpness", VuoReal_clamp(.5 - sharpness/2., 0, .5));
	...
	VuoShader_setUniform_VuoPoint2d((*instance)->shader, "gridType", (VuoPoint2d){
										(type==VuoGridType_Vertical   || type==VuoGridType_HorizontalAndVertical) ? 1 : 0,
										(type==VuoGridType_Horizontal || type==VuoGridType_HorizontalAndVertical) ? 1 : 0});
	...
	rotationMatrix[0] =  cc /  clampedSquareSize;
	rotationMatrix[1] =  cs / (clampedSquareSize * aspect);
	rotationMatrix[2] = -cs /  clampedSquareSize;
	rotationMatrix[3] =  cc / (clampedSquareSize * aspect);
