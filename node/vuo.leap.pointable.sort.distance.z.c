/**
 * @file
 * vuo.leap.pointable.sort.distance.z node implementation.
 *
...
VuoModuleMetadata({
					  ...
					  "description" :
						  "<p>Puts the pointables in order of nearest to farthest from a target point, \
...
)
{
	*sortedPointables = VuoListCreate_VuoLeapPointable();
	...
	for(int i = 0; i < count; i++)
		pointValues[i] = (sortable_pointValue){i, fabs(VuoListGetValueAtIndex_VuoLeapPointable(pointables, i+1).tipPosition.z - target.z)};
