...
VuoModuleMetadata({
					 "title" : "Get Scene",
					 "description" :
						"<p>Loads or downloads a 3D scene (3D model) from a URL.</p> \
						 <p>The vertices of the object should be in Vuo coordinates. \
						 If not, they can be scaled using the `Combine 3D Objects` node.</p> \
						 ...
						 <p>To download a 3D scene from the internet, copy the scene's URL from your browser. Example: <ul> \
						 <li>http://example.com/bunny.3ds</li> \
						 </ul></p> \
						 <p>To load a 3D scene from a file on the computer running the composition, create a URL from the scene file's path. Examples: <ul> \
						 <li>file:///Users/me/Desktop/bunny.dae</li> \
						 <li>file:///Users/me/file\\ with\\ spaces.dxf (for a file called `file with spaces.dxf`)</li> \
						 </ul></p> \
						 ...
						 than it was created on, then the 3D scene files need to be in the same location as on the original computer.</p> \
						 <p>Example 3D models are available at http://graphics.stanford.edu/data/3Dscanrep/ .</p>",
					 "keywords" : [
