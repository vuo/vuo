...
//    STB_TEXTEDIT_IS_SEPARATOR(ch)  true if character is whitespace (e.g. 'isspace'),
...
typedef struct
{
	...
	// pointer to rendered text coordinate mapping information.
	// caller is responsible for managing the lifecycle of this pointer.
	void* textImageData;
...
{
	state->cursor = stb_text_locate_coord(state, x, y);
...
// True on selection range chaged, false otherwise.
static bool stb_textedit_drag(STB_TEXTEDIT_STRING *str, STB_TexteditState *state, float x, float y)
{
	int o = state->select_end;
	int p = stb_text_locate_coord(state, x, y);
...
static void stb_textedit_key(STB_TEXTEDIT_STRING *str, STB_TexteditState *state, STB_TEXTEDIT_CHARTYPE key)
{
retry:
	switch (key) {
	default: {
		uint32_t c = STB_TEXTEDIT_KEYTOTEXT(key);
		if (c > 0) {
			STB_TEXTEDIT_CHARTYPE ch = (STB_TEXTEDIT_CHARTYPE) c;

			// can't add newline in single-line mode
			if (c == '\n' && (stb_textedit_linecount(str) >= state->line_count))
				break;
	...
	case STB_TEXTEDIT_K_SELECTALL:
		stb_textedit_clamp(str, state);
		state->cursor = state->select_start = STB_TEXTEDIT_STRINGLEN(str);
		state->select_end = 0;
		state->has_preferred_x = 0;
		break;
	...
	case STB_TEXTEDIT_K_DELETEWORD:
		if (STB_TEXT_HAS_SELECTION(state))
			stb_textedit_delete_selection(str, state);
		else
		{
			...
			if (state->cursor > 0)
			{
				int start = MAX(0, stb_textedit_move_to_word_previous(str, state));
				int end = state->cursor;
				stb_textedit_delete(str, state, start, end - start);
				state->cursor = start;
			}
		}
		state->has_preferred_x = 0;
	...
	case STB_TEXTEDIT_K_DELETEWORDRIGHT:
		if (STB_TEXT_HAS_SELECTION(state))
			stb_textedit_delete_selection(str, state);
		else
		{
			...
			if (state->cursor < len)
			{
				...
				int end = MAX(0, stb_textedit_move_to_word_next(str, state));
				stb_textedit_delete(str, state, start, end - start);
				state->cursor = start;
			}
		}
	...
	case STB_TEXTEDIT_K_DELETELINE:
		if (STB_TEXT_HAS_SELECTION(state))
		{
			stb_textedit_delete_selection(str, state);
		}
		else
		{
			...
			if (state->cursor > 0)
			{
				int end = state->cursor;
				int start = state->cursor;
				while(start > -1 && !VuoTextEdit_isNewLine(STB_TEXTEDIT_GETCHAR(str, start))) { start--; }
				start = MAX(0, start);
				stb_textedit_delete(str, state, start, end - start);
				state->cursor = start;
			}
	...
	case STB_TEXTEDIT_K_DELETELINERIGHT:
		if (STB_TEXT_HAS_SELECTION(state))
		{
			stb_textedit_delete_selection(str, state);
		}
		else
		{
			...
			if (state->cursor < len)
			{
				int start = state->cursor;
				int end = state->cursor;
				while(end <= len && !VuoTextEdit_isNewLine(STB_TEXTEDIT_GETCHAR(str, end))) { end++; }
				end = MIN(len, end);
				stb_textedit_delete(str, state, start, end - start);
				state->cursor = start;
			}
	...
	case STB_TEXTEDIT_K_DOWN | STB_TEXTEDIT_K_SHIFT: {
		...
		stb_textedit_clamp(str, state);
		stb_textedit_find_charpos(&find, str, state->cursor, state);
		...
		if (find.length) {
			...
			state->cursor = start;
			STB_TEXTEDIT_LAYOUTROW(&row, str, state->cursor, state);
			x = row.x0;
			// stop shy of num_chars because \n char is included, and for the cursor
			// to be on the next row it should be prior to the new line.
			for (i = 0; i < row.num_chars - 1; ++i) {
				float dx = STB_TEXTEDIT_GETWIDTH(str, start, i, state);
	...
	case STB_TEXTEDIT_K_UP | STB_TEXTEDIT_K_SHIFT: {
		...
		if (find.prev_first != find.first_char) {
			...
			// to be on the next row it should be prior to the new line.
			for (i = 0; i < row.num_chars - 1; ++i) {
				float dx = STB_TEXTEDIT_GETWIDTH(str, find.prev_first, i, state);
#ifdef STB_TEXTEDIT_GETWIDTH_NEWLINE
				if (dx == STB_TEXTEDIT_GETWIDTH_NEWLINE)
					break;
#endif
...
static void stb_textedit_clear_state(STB_TexteditState *state, int line_count)
{
...
// API initialize
static void stb_textedit_initialize_state(STB_TexteditState *state, int line_count)
{
	stb_textedit_clear_state(state, line_count);
}
