...
VuoModuleMetadata({
					  ...
					  "description" :
						  ...
						  comparing only the distance along the y-axis.</p> \
...
)
{
	*sortedPointables = VuoListCreate_VuoLeapPointable();
	...
	for(int i = 0; i < count; i++)
		pointValues[i] = (sortable_pointValue){i, fabs(VuoListGetValueAtIndex_VuoLeapPointable(pointables, i+1).tipPosition.y - target.y)};
