 ...
 */
extern "C" VuoRunner * VuoRunner_newSeparateProcessRunnerFromExecutable(const char *executablePath, const char *sourceDir,
																		bool continueIfRunnerDies, bool deleteExecutableWhenFinished)
{
	return VuoRunner::newSeparateProcessRunnerFromExecutable(executablePath, sourceDir, continueIfRunnerDies, deleteExecutableWhenFinished);
}
 ...
 *     VuoCompiler::linkCompositionToCreateDynamicLibraries(). The runner takes co-ownership of the VuoRunningCompositionLibraries
 ...
 */
extern "C" VuoRunner * VuoRunner_newCurrentProcessRunnerFromDynamicLibrary(const char *dylibPath, const char *sourceDir,
																		   bool deleteDylibWhenFinished)
{
	return VuoRunner::newCurrentProcessRunnerFromDynamicLibrary(dylibPath, sourceDir, deleteDylibWhenFinished);
}
...
VuoRunner::VuoRunner(void)
{
	...
	std::call_once(sleepHandlersInstalled, [](){
...
void VuoRunner::startInternal(void)
{
	...
	if (isInCurrentProcess())
	{
		...
		std::call_once(dylibLoggerInitialized, [](){
 ...
 * will have started.
 *
 * Assumes the same VuoRunningCompositionLibraries passed to VuoRunner::newSeparateProcessRunnerFromDynamicLibrary()
 * was passed again through VuoCompiler::linkCompositionToCreateDynamicLibrary() when linking the updated
 * version of the composition. If any subcompositions or other node classes are being replaced in this live-coding
 * reload, the VuoRunningCompositionLibraries also needs to be updated with a call to
 * VuoRunningCompositionLibraries::enqueueLibraryContainingDependencyToUnload().
 *
 ...
 * @param compositionDiff A comparison of the old and new compositions, produced by VuoCompilerCompositionDiff::diff().
...
void VuoRunner::replaceComposition(string compositionDylibPath, string compositionDiff)
{
	...
	dispatch_sync(controlQueue, ^{
					  ...
					  try
					  {
						  ...
						  vector<VuoRunningCompositionLibraries::CallbackType> doAfterLoadAttempted;
						  vector<string> dependencyDylibPathsRemoved = dependencyLibraries->dequeueLibrariesToUnload();
						  vector<string> dependencyDylibPathsAdded = dependencyLibraries->dequeueLibrariesToLoad(doAfterLoadAttempted);
						  ...
						  for (VuoRunningCompositionLibraries::CallbackType callback : doAfterLoadAttempted)
							  callback();
 ...
 * The composition will continue sending any telemetry subscribed by VuoRunner::subscribeToInputPortTelemetry(),
 * VuoRunner::subscribeToOutputPortTelemetry(), or VuoRunner::subscribeToAllTelemetry().
 *
 * Assumes the composition has been started and has not been stopped.
 *
...
json_object * VuoRunner::getPublishedOutputPortValue(VuoRunner::Port *port)
{
	...
	if (VuoRunner_isHostVDMX && port->getName() == "outputImage")
	{
		...
		{
			std::call_once(p->vuoImageFunctionsInitialized, [=](){
